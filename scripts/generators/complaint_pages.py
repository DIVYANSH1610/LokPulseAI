...
def generate_complaint_components(base_dir: str):
    wizard_content = """
"use client";
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div
              ...
              className="space-y-6"
            >
              <h2 className="text-2xl font-bold text-slate-800">What's the issue?</h2>
              <div className="grid grid-cols-2 gap-4">
                <button className="flex flex-col items-center gap-3 p-6 rounded-xl border-2 border-slate-200 hover:border-teal-500 hover:bg-teal-50 transition-all group">
                  ...
                  <span className="font-medium text-slate-700">Record Voice Note</span>
                </button>
                <button className="flex flex-col items-center gap-3 p-6 rounded-xl border-2 border-slate-200 hover:border-teal-500 hover:bg-teal-50 transition-all group">
                  <div className="p-4 bg-slate-100 rounded-full group-hover:bg-teal-100 text-slate-600 group-hover:text-teal-600">
                    <Camera size={24} />
          ...
          {step === 2 && (
            <motion.div
              ...
              className="space-y-6"
            >
              <h2 className="text-2xl font-bold text-slate-800">Where is this happening?</h2>
              <div className="bg-slate-100 rounded-xl h-48 flex items-center justify-center border-2 border-dashed border-slate-300">
                <button className="flex items-center gap-2 px-6 py-3 bg-white text-slate-700 rounded-lg shadow-sm hover:shadow font-medium">
                  <MapPin size={18} className="text-teal-600"/> Detect My Location
                </button>
              </div>
            </motion.div>
          )}
...
"""
    write_file(f"{base_dir}/src/components/citizen/ComplaintWizard.tsx", wizard_content)